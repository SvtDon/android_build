...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/evolution/vendorsetup.sh 2> /dev/null`
        do
echo "including $f"
            . $f
        done
...
echo "z$target" | grep -q "-"
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the EVOLUTION model name
            lunch evolution_$target-userdebug
        fi
fi
return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Evolution github
        T=$(gettop)
        pushd $T > /dev/null
